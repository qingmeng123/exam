...
func main() {
	...
	for i := 0; i < len(house.floor); i++ {
		indicator := house.floor[i].rooms[intRoom].indicator //进入房间指示牌
		code += indicator
		fmt.Println("房间指示牌", house.floor[i].rooms[intRoom].indicator)
		...
		for {
			...
			if key == indicator {
				fmt.Println("进入房间指示牌", key)
				intRoom = house.floor[i].rooms[intRoom].number
				fmt.Println("进入房间编号", intRoom)
				break
			}
			intRoom++
